...
установка по-прежнему запрещена) и объявляет права задачи. Неизвестная задача — fail-closed,
worktree не создаётся.

**Права объявляются в дереве lead-а, а не в worktree задачи**, и это не небрежность: hook берёт
`projectRoot` из `input.cwd`, а in-process subagent наследует cwd родительской сессии. Копия
внутри worktree остаётся человекочитаемой и не читается никогда. Поэтому `task:worktree`
обновляет `.claude/writeset.json` в дереве lead-а сам, а записи ЧУЖОЙ итерации удаляет с
предупреждением: в I03 уцелевшая запись прошлой итерации обнулила права всем четырём ревьюерам —
fail-closed на любой Bash, включая `pwd`, — и раунд верификации был потерян целиком, притом что
сессии работали и писали отчёты, которые никуда не дошли.
