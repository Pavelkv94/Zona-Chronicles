...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      // `apps/web` добавлен по M7 независимого аудита I03 и воспроизведён его же фикстурой:
      // проба «apps/web -> packages/persistence» давала НОЛЬ нарушений, тогда как та же проба от
      // `apps/api` — 27. Правило было написано, когда observer-путь состоял из одного приложения;
      // экран появился в этой же итерации и оказался вне запрета, хотя он такой же observer-путь.
    ...
    },
    {
      ...
      severity: 'error',
      comment:
        'ADR-002 не даёт ребра app -> app: приложения общаются через packages, а не напрямую. ' +
        'Правила не было вовсе (m12 независимого аудита I03), и это не теория: `PLAN.md` §10.2 ' +
        'опирается на отсутствие такого ребра как на ИСПОЛНЯЕМОЕ ограничение — там перечислены ' +
        'отвергнутые варианты, и первый отвергнут словами «прямое нарушение границы приложений». ' +
        'Ограничение, на которое ссылается решение, обязано быть проверяемым, иначе следующий ' +
        'выбор сделают по памяти. Относительный импорт ловится так же, как пакетный: правило ' +
        'смотрит на путь модуля, а не на форму записи.',
      from: { path: '^apps/([^/]+)/' },
