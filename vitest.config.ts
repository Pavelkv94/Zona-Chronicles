    ...
    projects: [
      ...
      },
      {
        ...
        test: {
          ...
          hookTimeout: 300_000,
          /**
           * Файлы идут ПО ОЧЕРЕДИ, а не параллельно, и это не осторожность, а условие
           * измеримости.
           *
           * Acceptance-тесты здесь порождают ОС-процессы и работают с настоящей базой: A1 делает
           * 100 запусков CLI на каждый из четырёх seed, D6 убивает worker-ы, M4 обрывает
           * `world init` шестнадцать раз, M1 поднимает worker со сломанной проекцией. Пока файлов
           * было мало, параллельность стоила недорого. В I03 их стало двенадцать, и утверждения,
           * ОГРАНИЧЕННЫЕ ВРЕМЕНЕМ, начали падать от соседей, а не от дефектов.
           *
           * Измерено: одно порождение CLI на свободной машине — около 800 мс, то есть 100
           * прогонов A1 укладываются в 80 секунд при бюджете 200. Под параллельной нагрузкой тот
           * же тест потратил 246 секунд и упал по таймауту. Дефекта не было ни в нём, ни в
           * продукте — были соседи.
           *
           * Тот же довод, по которому `playwright.config.ts` держит `workers: 1`: сценарии делят
           * один набор ресурсов, и параллельный прогон даёт ровно тот класс ложных падений, когда
           * падает не тест, а его сосед.
           */
          fileParallelism: false,
        },
      },
