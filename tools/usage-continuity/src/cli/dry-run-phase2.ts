#!/usr/bin/env node
/**
 * Phase 2 двухпроцессного dry-run (DEV-01, review m5, B3): НОВЫЙ процесс, не разделяющий
 ...
 *  - `progressed`      — первый resume после wake ЧИТАЕТ checkpoint (`store.read`, ранее нигде
 *                        не вызывавшийся вне собственных тестов) и выполняет next_exact_action
 *                        ровно один раз, пройдя ветку `resetReached` (`now >= reported_reset_at`),
 *                        а НЕ ветку `recovered` — m5.
 *  - `resume_conflict` — повторный resume в ЕЩЁ ОДНОМ новом процессе видит персистентный маркер
 *                        `resume_attempted_at` и отказывается выполнять действие повторно.
 *
 * Использование: node dry-run-phase2.ts <checkpoint-store-root> <progressed|resume_conflict>
 * Код возврата: 0 — фактический outcome совпал с ожидаемым и action выполнен ожидаемое число
 *               раз (1 для progressed, 0 для resume_conflict); 1 — иначе.
 */
...
if (root === undefined || (expectArg !== 'progressed' && expectArg !== 'resume_conflict')) {
  console.error('usage: dry-run-phase2.ts <checkpoint-store-root> <progressed|resume_conflict>');
  process.exit(1);
}
