...
| PR-04 | Мир существует и меняется без открытого браузера | [[01_PRODUCT_DESIGN]] §1, §4 | worker, scheduler, PostgreSQL event log | I02B — **впервые буквально**: путь, начатый командой, завершается worker-ом сам, мировое время двигают события (`world tick`, C2), остановка между стартом и завершением ничего не теряет и не дублирует (C7, настоящие процессы). Остаётся I15B: 7-day soak |
...
| OPS-01 | Crash/restart не теряет и не дублирует canonical работу | [[03_TECHNICAL_DESIGN]] §3–5 | transaction event/state/outbox; leases; snapshots | I02A — **выполнено частично**: одна транзакция event/state/outbox/command result, инъекция сбоя после каждого шага записи (B5), идемпотентность по `command_id` включая устаревшую версию (B3), конкурентные команды без дыр в `sequence` (B6), состояние переживает перезапуск процесса (B9). Остаётся I02B: lease/reclaim worker-а, snapshots и replay  I02B — добавлено: lease с истечением и reclaim (C6), конкурентные worker-ы через `SKIP LOCKED` без дублей и дыр в `sequence` (C5), стабильный порядок обработки при равном `due_at` (C4), монотонность мирового времени (C12). Остаётся: replay-доказательство (C9–C10) |
