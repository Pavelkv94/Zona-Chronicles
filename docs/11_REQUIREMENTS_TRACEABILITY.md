...
| PR-04 | Мир существует и меняется без открытого браузера | [[01_PRODUCT_DESIGN]] §1, §4 | worker, scheduler, PostgreSQL event log | I02B restart/replay; I15B 7-day soak |
...
| OPS-01 | Crash/restart не теряет и не дублирует canonical работу | [[03_TECHNICAL_DESIGN]] §3–5 | transaction event/state/outbox; leases; snapshots | I02A–I02B; fault injection и restart demo |
...
| DEV-01 | При остатке пятичасового Claude usage window `<= 1%` работа checkpoint-ится, ждёт reset и автоматически продолжается без потери/дублирования шага | [[08_TDD_AND_AGENT_WORKFLOW]] §9; `CLAUDE.md` | injected usage telemetry, persisted checkpoint, external scheduled wake/session resume | I00; fake `2% → 1% → reset` dry-run без ручного повторного prompt |
