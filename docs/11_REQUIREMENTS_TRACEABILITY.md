...
| OPS-03 | Observer/admin/worker/migration и CI/release границы имеют deny-by-default, least privilege и проверяемый software supply chain | [[03_TECHNICAL_DESIGN]] §11–12; ADR-008 | threat model, DB/network roles, ASVS applicability, scans, SBOM/checksum/provenance | I00 skeleton; I02A — разделение PostgreSQL-ролей исполнено грантами и проверено отказами: `world_events` append-only для canonical worker, read-only api не имеет доступа к каноническим таблицам, application-роли без DDL (B7). I17 release gate |
