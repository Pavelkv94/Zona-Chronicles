...
### Идемпотентность: что обязан делать клиент (I02A)

`command_id` — ключ идемпотентности. Правила, выведенные реализацией I02A и обязательные для
ЛЮБОГО клиента (CLI, HTTP API, scheduler):

1. **Повтор — это тот же `command_id` И то же тело.** Совпадение полей отпечатка
   (`COMMAND_FINGERPRINT_KEYS` в `@zona/contracts`) проверяется на записи: чужое тело под
   занятым идентификатором получает `precondition_failed` и попадает в аудит попыток, а не
   принимается как повтор.
2. **Трассировочные поля различаться МОГУТ.** `correlation_id` и `issued_at_world_time` в
   отпечаток не входят, поэтому ретрай, пересобранный со свежим id трассировки, остаётся
   идемпотентным. Требовать от клиента побайтового воспроизведения трассировки нельзя.
3. **Новая попытка после отказа требует НОВОГО `command_id`.** Отказ журналируется так же, как
   принятие, поэтому повтор с тем же идентификатором вечно возвращает записанный отказ. Для
   ретраибельного по смыслу состояния — например `stale_world_version` — клиент обязан взять
   новый `command_id`, а не повторять прежний.
4. **Без явного `command_id` каждый вызов — новая попытка.** Выводить идентификатор из
   изменяемого состояния мира запрещено: тогда повтор после потерянного ответа получает другой
   id и проходит мимо журнала идемпотентности, то есть OPS-01 не выполняется.
