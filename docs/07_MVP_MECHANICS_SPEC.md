...
### Как нужда меняется во времени (решение I04)

Нужда — **производная величина, а не факт**. Каноническим состоянием является момент мирового
времени, с которого нужда отсчитывается (последний приём пищи, последний отдых); само значение
вычисляется чистой функцией от него, текущего мирового времени и коэффициентов ruleset.

Прежняя редакция этого параграфа говорила: «нужды обновляются scheduled pulse-ами, но событие
публикуется только при пересечении значимого порога». Прямое прочтение противоречит постоянному
правилу проекта: canonical state меняется ТОЛЬКО событиями. Pulse, меняющий голод и не
публикующий события, ломает replay — состояние перестаёт быть функцией журнала. Pulse,
публикующий событие каждый раз, даёт ~96 событий в сутки на агента при шаге 15 минут и упирается
в собственное STOP-условие итерации I04 («pulses создают непросматриваемый поток событий»).

Поэтому pulse-ов нет вовсе. Момент следующего пересечения порога ВЫЧИСЛИМ заранее из той же
функции и тех же коэффициентов, поэтому он планируется обычным `ScheduledAction` — тем же
механизмом, что прибытие по маршруту. Между порогами не происходит ничего: ноль событий, ноль
изменений canonical state, ноль записей в журнал. На каждое пересечение — ровно одно событие.

Следствия, за которые это выбрано:

- журнал остаётся списком фактов, а не выборкой измерений;
- `canonical_state` между событиями не меняется, поэтому checksum и replay не затронуты;
- event spam невозможен КОНСТРУКТИВНО, а не благодаря удачно выбранному шагу pulse;
- Utility AI (§6) читает значение нужды в момент решения, и оно детерминировано.

Цена названа честно: функция нужды обязана быть монотонной и обратимой на участке между
порогами, иначе «когда наступит следующий порог» не вычислить. Для линейного роста это
выполняется. Переход к нелинейной функции потребует либо кусочной линеаризации, либо возврата к
pulse-ам вместе с их ценой.
...
### Как выбор происходит на самом деле (решение I05)

Формула выше — цель, а не текущее состояние. Реализовано то, для чего в мире ЕСТЬ входы, и
здесь записано, что именно.

- **Слагаемых три:** `urgency - time_cost - switching_cost`. `expected_value` не введён не по
  объёму работ: цели этого среза снимают свою нужду полностью, поэтому ожидаемая польза совпала
  бы со срочностью до последней тысячной — это не слагаемое, а второе имя того же числа.
  Остальные слагаемые вводятся вместе со своими входами (черты — I06, риск — I07, отношения и
  обязательства — I09).
- **Гистерезис и `switching_cost` — один механизм.** Вычесть `switch_margin` из каждого
  кандидата, отличного от текущей цели, и взять максимум — тождественно правилу «новая цель
  обязана превзойти текущую на margin». Заводить их порознь значит вычесть порог дважды.
- **Все величины целые, в тысячных.** Каноническая сериализация отвергает дробное число, а
  коэффициенты входят в rules bundle, checksum которого обязан совпадать на любой машине.
- **Срочность считается по УРОВНЮ нужды, а не по её значению.** Значение — производная величина
  и измерение; фактом является уровень (§5). Решение, принятое по измерению, менялось бы между
  двумя соседними минутами без единого факта, который это объяснил бы.
- **Исполнимость входит в отбор кандидатов.** Цель, для которой нельзя построить первый шаг
  здесь и сейчас, кандидатом не является. «Поесть» без еды — не цель с низкой оценкой, а вовсе
  не цель. Иначе агент выбирал бы недостижимое, оставался свободным и получал новое решение —
  мир крутился бы, не породив ни одного факта.
- **Ничья решается порядком объявления целей**, а не порядком перебора: второе зависело бы от
  того, как написан цикл. `idle` объявлен первым — при равенстве мир предпочитает бездействие.
- **Агент решает, когда для НЕГО что-то изменилось**, и только будучи свободным. Периодического
  опроса нет: «каждые N минут пересчитать цели» — это pulse под другим именем, со всей ценой из
  §5. Триггером всегда является факт.
- **Решение — это факт, и оно записывается всегда**, даже когда цель не изменилась. Причина
  техническая и названа в `PLAN.md` §12 итерации I05: отвергнутое запланированное действие
  остаётся в каноническом состоянии навсегда. Побочный выигрыш содержательный: разбор решения,
  оставшегося праздным, — единственное место, где видно, почему голодающий агент ничего не
  предпринял.
- **Цель короткоживущая: она потребляется своим шагом.** Поевший агент снова празден — не
  потому, что передумал, а потому, что цель достигнута.

### Срыв плана (решение I05-C)

- **Невыполненное предусловие даёт СРЫВ ПЛАНА, когда план есть, и обычный доменный отказ, когда
  его нет.** Различие мир проводит по СОСТОЯНИЮ агента, а не по происхождению команды:
  происхождения команда не несёт и нести не должна. Разница существенная — отказ лишь помечает
  действие конечным и оставляет агента с целью, которую некому исполнить; событие снимает цель,
  освобождает агента и назначает новое решение.
- **У плана есть тождество** (`plan_id`), выведенное из факта, который план породил. Без него два
  подряд сорванных плана одного агента неотличимы, и вопрос «сорвался дважды или сорвались два
  разных» остаётся без ответа.
- **Emergency interrupt прерывает та нужда, которую план НЕ лечит.** Усталость, дошедшая до
  предела во время отдыха, — это не происшествие, а ровно то, чем агент занят: прерывать отдых
  из-за усталости значило бы поднимать спящего, чтобы отправить его спать. Голод во время отдыха
  — другое дело.
- **Прерванное занятие заканчивается ничем.** Момент отсчёта нужды при срыве не сдвигается: плата
  за работу, которой не было, — самая тихая из возможных ошибок.
- **Путь сорванным планом не отменяется:** идущий находится НА ДОРОГЕ, и прерванный путь оставил
  бы его между местами — положения, которого мир не выражает. (До I06-C основанием служило «маршрут
  выбирает оператор, а не агент»; с появлением цели «уйти» дорогу выбирает агент, а вывод остался
  прежним по другой причине.)

### Короткий план

План первого среза содержит РОВНО ОДИН шаг (решение I05): шагов длиннее пока не из чего
строить — «дойти до еды» требует источника предметов (I08), «помочь» — отношений (I09).
Обещанный ниже предел остаётся целью.
...
#### Как это работает на самом деле (решение I06)

Реализовано то, для чего в мире ЕСТЬ входы: `duration + perceived_risk * caution`.
`resource_cost` и `goal_relevance` не введены — их не из чего считать.

- **Неизвестность выражена ПРЕДПОЛАГАЕМЫМ риском, а не отдельным штрафом.** `uncertainty_penalty`
  и `perceived_risk` неизвестной дороги — одна и та же величина; два множителя за одно и то же
  взяли бы двойную плату. Осторожность уже умножает воспринимаемый риск, поэтому неизвестная
  дорога дорожает для пугливого агента сама собой.
- **Незнание — это отсутствие записи, а не ноль.** Подставить ноль значило бы объявить весь мир
  заведомо безопасным для того, кто ничего не видел.
- **Осторожность умножает риск, а не всю цену.** Бояться можно опасности, а не расстояния;
  умножение всей цены сделало бы пугливого агента ещё и домоседом, и различить эти два свойства
  в поведении стало бы невозможно.
- **Предполагаемый риск обязан оставлять уход возможным.** Слишком высокое значение даёт мир, где
  никто не трогается с места, пока не узнает дорогу, — а узнать её, не пройдя, нельзя.
- **Ничья решается ключом дороги**, а не порядком перебора: второе зависело бы от того, как
  собран список.
- **Опасность МЕСТА агент знает без памяти** — он там стоит. Опасность ДОРОГИ требует того, чтобы
  по ней прошли: знание входит в мир фактом `risk.observed` и принадлежит агенту, а не миру.
- **Уйти — это цель** (`flee`), а не отдельная механика. Её срочность — опасность места, её цена —
  цена лучшей доступной дороги. Решить уйти, не зная куда, нельзя: «уйти» и «выбрать дорогу» —
