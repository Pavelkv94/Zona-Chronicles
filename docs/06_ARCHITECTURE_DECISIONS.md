...
Ожидаемое, но НЕ обязательное до I14B («Observer UX: понять мир»):
...
Разделение введено в I03 по m15 независимого архитектурного аудита. Экран этой итерации написан
на React с обычным CSS и `fetch`/`EventSource`: карта из четырёх узлов — это список, а не
геовизуализация, и ставить MapLibre ради неё значило бы принести зависимость под задачу, которой
нет. То же с TanStack Query: запросов два, и оба обслуживаются десятком строк.

Формально это расходилось с перечнем выше, а перечень не различал «решено» и «предполагается» —
поэтому расхождение выглядело нарушением ADR, хотя решением было ровно обратное: не тащить в
прототип стек раньше, чем появится задача, которая его оправдывает. Список приведён в
соответствие с тем, что действительно решено; выбор конкретных библиотек карты и кэширования
делается в I14B, вместе с первой настоящей визуализацией.
...
**Где именно проверяется** (уточнение M3, аудит I02B; поправка M-C, второй раунд верификации).
До правки эта фраза описывала контроль, которого в работающей системе не было: функция
существовала и была покрыта unit-тестами, но не вызывалась ни на одном пути. Точка проверки —
ЧТЕНИЕ снимка (`loadLatestSnapshot`/`loadSnapshotAt` в
`packages/persistence/src/snapshot-store.ts`).

**Эта точка была НЕ полна, и формулировка «именно там мир продолжается процессом с другим
профилем» была неверной.** Канонический мир изменяет `executeCommand`, который снимков не читает
вовсе: процесс с неквалифицированным профилем беспрепятственно дописывал канонические события, а
`world replay` — единственный детектор расхождения детерминизма — именно в этот момент переставал
запускаться. Детектор выключался ровно тогда, когда нужен.

**Исправлено в I03.** Мир записывает профиль, под которым квалифицирован
(`worlds.qualified_runtime_profile`, миграция 0012, backfill из генезисного снимка), а каждый
канонический писатель проходит `qualifyCanonicalWriter` перед первой записью. Отказы различимы:
мира нет; профиль не записан (мир создан до контроля — сравнить не с чем, и это не «всё в
порядке»); профиль несовместим (§7 — «не квалифицирован», а не «сломано»).

Гранулярность — ОДИН РАЗ НА ПРОЦЕСС, а не на команду: профиль не меняется между двумя командами
одного запуска, а обязательный параметр в `executeCommand` протащил бы его через 79 мест вызова
ради заведомо одинакового значения. **Остаточный риск записан, а не спрятан:** это шлюз, а не
замок — код, открывший соединение в обход шлюза, останется непроверенным. Сегодня писателей ровно
два (`apps/worker`, `apps/cli`), оба его проходят. Обязательный токен в сигнатуре `executeCommand`
оправдан, когда писателей станет больше двух, и вынесен требованием в I04.

Промежуточно закрыто одно следствие: процедура §7 («сравнить replay и resimulation regression
bank на старом и новом profile») была НЕИСПОЛНИМА через поставляемый API, потому что загрузка
старого снимка бросала. Явный `world replay --accept-unqualified-profile` делает её исполнимой,
не меняя умолчания и печатая предупреждение отдельной строкой. Профиль читающего процесса передаёт вызывающий
