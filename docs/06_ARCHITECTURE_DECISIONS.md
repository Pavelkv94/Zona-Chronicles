...
**Эта точка НЕ полна, и формулировка «именно там мир продолжается процессом с другим профилем»
была неверной.** Канонический мир изменяет `executeCommand`, который снимков не читает вовсе:
процесс с неквалифицированным профилем беспрепятственно дописывает канонические события, а
`world replay` — единственный детектор расхождения детерминизма — именно в этот момент перестаёт
запускаться. Детектор выключается ровно тогда, когда нужен. Полное исправление — симметричная
проверка на пути ЗАПИСИ — требует durable «квалифицированного профиля мира» (ещё колонка и
миграция) и вынесено требованием в I03.
