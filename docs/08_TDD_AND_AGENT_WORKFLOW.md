...
3. **Проверять ОБА направления.** Проба обязана показать и что нарушение ловится, и что
   корректный код проходит. Контроль, который срабатывает всегда, неотличим от работающего,
   пока смотришь только на первое направление: в I02B правило границ для replay падало и на
   правильной реализации, а проба этого не увидела, потому что проверяла лишь срабатывание.
