...
/**
 * Оценка цели в utility AI (I05, `07_MVP_MECHANICS_SPEC` §6).
 *
 * Minor unit — тысячная, как у нужды: слагаемые оценки выводятся из уровней нужд, и держать их
 * в разных масштабах значило бы делить одно на другое при каждом сравнении.
 *
 * Диапазон шире `[-1000, 1000]` намеренно. Оценка — это РАЗНОСТЬ: срочность минус цена времени
 * минус цена смены цели, и цена времени растёт с длительностью действия. Границы обязаны
 * покрывать любой ruleset, проходящий свою проверку, иначе property-тест «оценка лежит в
 * объявленном диапазоне» проверял бы не функцию, а удачно подобранные коэффициенты.
 */
export const GOAL_SCORE_UNIT = defineNumericUnit({
  id: 'score.goal',
  description: 'Оценка цели utility AI; minor unit — тысячная (3 знака).',
  minorUnitsPerMajor: 1000,
  min: -1_000_000,
  max: 1_000_000,
});
