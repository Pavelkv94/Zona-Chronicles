 ...
 *
 * Публикуется ВСЯКИЙ РАЗ, когда агент решал, — включая случай «подумал и остался при своём».
 *
 * Замысел был обратным: писать факт только когда решение изменило мир, чтобы летопись не
 * наполнялась пустыми записями. Он оказался невыполним, и причина лежит не в эстетике, а в
 * состоянии (`PLAN.md` I05 §12.1). Отвергнутое запланированное действие остаётся в каноническом
 * состоянии навсегда — ограничение `scheduled_actions_single_outcome` требует у каждого действия
 * ровно один исход, — поэтому «решил ничего не менять» обязано быть исходом, а не молчанием.
 * Молчание давало расхождение при пересимуляции: журнал не объяснял, куда делось назначенное
 * решение.
 *
 * Цена названа прямо: в ленте есть строки, ничего не меняющие. Это честнее, чем журнал, из
 * которого мир не восстанавливается.
 *
