...
describe('command envelope v1 (§2)', () => {
  ...
  it('перечисляет ровно объявленные типы команд и ничего сверх них (§11, §12)', () => {
    ...
    expect([...COMMAND_TYPES]).toEqual([
      ...
      // I04: агент ест и отдыхает. `agent.eat` формирует сам мир по достигнутому порогу,
      // `agent.rest` пока подаёт оператор — выбор цели это I05.
      'agent.eat',
      'agent.rest',
      // I05: завершение отдыха формирует расписание — тем же путём, что завершение пути.
      'rest.complete',
    ]);
  });
