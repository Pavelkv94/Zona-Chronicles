...
describe('command envelope v1 (§2)', () => {
  ...
  it('перечисляет ровно объявленные типы команд и ничего сверх них (§11, §12)', () => {
    ...
    expect([...COMMAND_TYPES]).toEqual([
      ...
      // I04: агент ест и отдыхает. С I05-B обе команды формирует сам мир — как шаги
      // выбранной цели; человек их не подаёт и подавать не может.
      'agent.eat',
      'agent.rest',
      // I05-A: завершение отдыха формирует расписание — тем же путём, что завершение пути.
      'rest.complete',
      // I05-B: агент выбирает цель. Payload пуст: кандидаты выводятся из состояния мира и
      // ruleset, а не приходят входом команды.
      'agent.decide',
      // I06-C: агент уходит от опасности. Маршрут в payload не назван: его выбирает `decide` по
      // субъективной карте риска, а не тот, кто сформировал команду.
      'agent.travel',
    ]);
  });
