  ...
  return withDecisionScheduled(
    {
      ...
      agents: {
        ...
        [actorId]: {
          ...
          planId: null,
          /**
           * Путь сорванным планом не отменяется — но уже НЕ ПОТОМУ, что маршрут задал оператор.
           *
           * Прежнее обоснование («маршрут выбирает оператор, а не агент») умерло в I06-C, когда
           * дорога стала шагом цели «уйти»: теперь маршрут выбирает агент. Поведение при этом
           * верное, и основание у него другое — идущий по дороге НАХОДИТСЯ на ней. Отменить путь
           * значило бы вернуть агента в точку выхода или оставить между местами; ни того, ни
           * другого мир выразить не умеет, и сорванный план такого права не даёт.
           *
           * Найдено независимым ревью I04-I06 (m2). Починка пережила свою предпосылку молча —
           * ровно тот случай, ради которого обоснования и записываются рядом с кодом.
           */
...
function applyRiskObserved(
  ...
  /**
   * ПЕРВОЕ наблюдение остаётся, повторное ничего не меняет.
   *
   * Первая редакция перезаписывала запись, и это расходилось с путём записи: он умеет только
   * INSERT (`command-handler.ts`), а гранты на `agent_route_knowledge` — `SELECT`/`INSERT`,
   * без `UPDATE`. Два слоя реализовывали РАЗНЫЕ правила применения одного события, и согласие
   * между ними держала третья сторона — подавление повтора в `decide`. Найдено независимым
   * test-review I04-I06 (M2); проба «снять подавление» падала не на составе летописи, а на
   * страже checksum, то есть по симптому, а не по причине.
   *
   * Правило выбрано не произволом: обновление знания — это вопрос УСТАРЕВАНИЯ, а он решается в
   * I10A вместе с пересказом и доверием к источнику. Пока решения нет, два слоя не имеют права
   * кодировать разные ответы; когда оно появится, обратная сторона придёт вместе с фактом,
   * который её порождает, и с миграцией, дающей право на UPDATE.
   */
  if (agent.knownRoutes[event.payload.route_id] !== undefined) return state;
