...
describe('M4 — обрыв world init не оставляет мир, который нельзя ни использовать, ни удалить', () => {
  ...
  it('после любого обрыва мир либо есть целиком, либо его нет вовсе', async () => {
    ...
    /**
     * Если ни один обрыв не дал полного мира, доводим создание БЕЗ обрыва.
     *
     * Полный gate показал, почему это нужно: под нагрузкой `world init` идёт дольше, все
     * шестнадцать обрывов ложатся ДО завершения, и защита от бессмысленного прогона краснела —
     * справедливо по букве и бесполезно по существу. Исход «мир создан целиком» нужен не как
     * результат удачного обрыва, а как вторая точка отсчёта: она доказывает, что моменты обрыва
     * легли по обе стороны операции, то есть окно записи пересечено.
     *
     * Тот же класс, что в тесте D6, только там его удалось убрать наблюдением за ростом ленты.
     * Здесь наблюдать нечего по построению: незакоммиченную транзакцию снаружи не видно — в этом
     * и состоит проверяемое свойство.
     */
    if (!outcomes.some((shape) => shape.meta)) {
      const finished = spawnWorldCliDirect(['world', 'init', '--seed', '42'], {
        env: { DATABASE_URL: db.url },
      });
      expect(finished.exitCode, finished.stdout).toBe(0);
      const shape = await inspect();
      expect(shape.meta && shape.snapshot, 'непрерывный world init не создал мир целиком').toBe(
        true,
      );
      outcomes.push(shape);
    }
