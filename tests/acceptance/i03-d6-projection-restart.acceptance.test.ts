...
describe('I03 D6 — убитый сборщик проекции догоняет журнал ровно один раз на событие', () => {
  ...
  it('D6: три SIGKILL посреди догона не дают ни пропусков, ни дублей', async () => {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      // Пауза подобрана так, чтобы попасть в фазу догона, а не строго в транзакцию: тест
      // требует верного РЕЗУЛЬТАТА при любом попадании, а не конкретного попадания.
    ...
    // работой — например, процесс не успевал стартовать за отведённую паузу, — тест проходил бы,
    // ничего не проверив: последний, неубитый запуск собрал бы всё с нуля, и результат сошёлся
    // бы независимо от того, атомарен шаг или нет. Поэтому требуем, чтобы к этому моменту
    // проекция была ЧАСТИЧНОЙ: что-то записано и записано не всё.
