...
test('D3: десять зрителей не меняют того, что произвёл мир', async ({ page }) => {
   ...
   *
   * `goal.chosen` отфильтрован намеренно: с I05-B прибывший агент СВОБОДЕН, и мир немедленно
   * даёт ему выбрать, что делать дальше. Это правильное поведение, но к утверждению D3 — «число
   * зрителей не меняет того, что произвёл мир» — отношения не имеет: здесь сравнивается путь.
  ...
  const legOf = (events: string, agentId: string) =>
    events
      .split('\n')
      .filter((line) => line.includes(agentId) && !line.includes('goal.chosen'))
      .map((line) => {
